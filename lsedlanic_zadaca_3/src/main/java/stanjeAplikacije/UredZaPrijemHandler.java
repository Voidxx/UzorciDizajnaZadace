...
public class UredZaPrijemHandler implements Handler{
	  private Handler next;

	  @Override
	  public void handleSave(Map<String, Object> state) {
	   Field[] fields = UredZaPrijem.getInstance().getClass().getDeclaredFields();
	   for (Field field : fields) {
	      try {
	          field.setAccessible(true);
	          String name = field.getName();
	          Object value = field.get(UredZaPrijem.getInstance());
	      ...
	      try {
	          ...
	          if (value != null) {
	        	  if (value instanceof List) {
	        		   ...
	        		   field.set(UredZaPrijem.getInstance(), listValue);
	        		} else {
	        		   field.set(UredZaPrijem.getInstance(), value);
	        		}
	          }
	      } catch (IllegalAccessException e) {
	          e.printStackTrace();
