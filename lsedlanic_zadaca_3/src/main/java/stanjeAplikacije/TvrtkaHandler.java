...
public class TvrtkaHandler implements Handler {
	  private Handler next;

	  @Override
	  public void handleSave(Map<String, Object> state) {
	   Field[] fields = Tvrtka.getInstance().getClass().getDeclaredFields();
	   for (Field field : fields) {
	      try {
	          field.setAccessible(true);
	          String name = field.getName();
	          Object value = field.get(Tvrtka.getInstance());
	      ...
	      try {
	          ...
	          if (value != null) {
	        	  if (value instanceof List) {
	        		   ...
	        		   field.set(Tvrtka.getInstance(), listValue);
	        		} else {
	        		   field.set(Tvrtka.getInstance(), value);
	        		}
	          }
	      } catch (IllegalAccessException e) {
	          e.printStackTrace();
