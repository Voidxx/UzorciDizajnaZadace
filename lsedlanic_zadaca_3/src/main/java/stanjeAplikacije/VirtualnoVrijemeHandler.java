...
public class VirtualnoVrijemeHandler implements Handler {
	   private Handler next;
	   
		  @Override
		  public void handleSave(Map<String, Object> state) {
		   Field[] fields = VirtualnoVrijeme.getInstance().getClass().getDeclaredFields();
		   for (Field field : fields) {
		      try {
		          field.setAccessible(true);
		          String name = field.getName();
		          Object value = field.get(VirtualnoVrijeme.getInstance());
		      ...
		      try {
		          ...
		          if (value != null) {
		        	  if (value instanceof List) {
		        		   ...
		        		   field.set(VirtualnoVrijeme.getInstance(), listValue);
		        		} else {
		        		   field.set(VirtualnoVrijeme.getInstance(), value);
		        		}
		          }
		      } catch (IllegalAccessException e) {
		          e.printStackTrace();
